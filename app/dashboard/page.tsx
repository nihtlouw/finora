...
export default async function DashboardPage() {
 ...
 const dailyBuckets=Array.from({length:14},(_,index)=>{
   ...
   return {
     ...
     income:payments.filter(x=>{const d=new Date(x.paymentDate);return d.getFullYear()===date.getFullYear()&&d.getMonth()===date.getMonth()&&d.getDate()===date.getDate()}).reduce((s,x)=>s+Number(x.amount),0),
     expense:expenses.filter(x=>{const d=new Date(x.expenseDate);return d.getFullYear()===date.getFullYear()&&d.getMonth()===date.getMonth()&&d.getDate()===date.getDate()}).reduce((s,x)=>s+Number(x.amount),0),
   }
 })

 const weekAnchor=new Date(today)
 const dayOfWeek=weekAnchor.getDay()
 weekAnchor.setDate(weekAnchor.getDate()+(dayOfWeek===0?-6:1-dayOfWeek))

 const weeklyBuckets=Array.from({length:8},(_,index)=>{
   const start=new Date(weekAnchor)
   start.setDate(weekAnchor.getDate()-(7-index)*7)
   ...
   return {
     date:start,
     label:`${new Intl.DateTimeFormat('id-ID',{day:'2-digit',month:'short'}).format(start)}–${new Intl.DateTimeFormat('id-ID',{day:'2-digit',month:'short'}).format(endLabel)}`,
     income:payments.filter(x=>{const d=new Date(x.paymentDate);return d>=start&&d<end}).reduce((s,x)=>s+Number(x.amount),0),
     expense:expenses.filter(x=>{const d=new Date(x.expenseDate);return d>=start&&d<end}).reduce((s,x)=>s+Number(x.amount),0),
   }
 })

 const monthAnchor=new Date(today.getFullYear(),today.getMonth()-5,1)
 const monthlyBuckets=Array.from({length:6},(_,index)=>{
   const start=new Date(monthAnchor.getFullYear(),monthAnchor.getMonth()+index,1)
   const end=new Date(start.getFullYear(),start.getMonth()+1,1)
   return {
     date:start,
     label:new Intl.DateTimeFormat('id-ID',{month:'short',year:'numeric'}).format(start),
     income:payments.filter(x=>{const d=new Date(x.paymentDate);return d>=start&&d<end}).reduce((s,x)=>s+Number(x.amount),0),
     expense:expenses.filter(x=>{const d=new Date(x.expenseDate);return d>=start&&d<end}).reduce((s,x)=>s+Number(x.amount),0),
   }
 })
 ...
 const activities=[
  ...
  <div className="f-kpi-row"><section className="f-card dashboard-cashflow-card"><div className="f-card-head"><div><h3>Arus Kas</h3><p>6 bulan terakhir · pembayaran masuk dan biaya yang disetujui.</p></div><span className="f-badge green"><i className="f-live-dot"/>Live dari database</span></div><CashFlowChart dataByMode={{
 daily:dailyBuckets.map(b=>({label:b.label,income:b.income,expense:b.expense})),
 weekly:weeklyBuckets.map(b=>({label:b.label,income:b.income,expense:b.expense})),
 monthly:monthlyBuckets.map(b=>({label:b.label,income:b.income,expense:b.expense})),
}}/></section>

...
const navGroups = [
 {label:"Work",items:[
  {href:"/dashboard",label:"Dashboard",icon:"dashboard" as IconName},{href:"/clients",label:"Klien & Vendor",icon:"users" as IconName},
  {href:"/proposals",label:"Proposal",icon:"file" as IconName},{href:"/customer-pos",label:"PO Customer",icon:"clipboard" as IconName},
  {href:"/projects",label:"Project",icon:"folder" as IconName},{href:"/change-orders",label:"Change Orders",icon:"arrows" as IconName}]},
 {label:"Money",items:[
  {href:"/invoices",label:"Invoice",icon:"receipt" as IconName},{href:"/receivables",label:"Piutang",icon:"wallet" as IconName},
  {href:"/payments",label:"Pembayaran",icon:"arrows" as IconName},{href:"/credit-notes",label:"Credit Note",icon:"minus" as IconName},
  {href:"/expenses",label:"Biaya",icon:"coins" as IconName},{href:"/cashflow",label:"Cash Flow",icon:"arrows" as IconName},
  {href:"/budgets",label:"Anggaran",icon:"chart" as IconName},{href:"/vendor-bills",label:"Vendor Bills",icon:"receipt" as IconName},
  {href:"/payroll",label:"Payroll",icon:"users" as IconName},{href:"/banks",label:"Bank",icon:"wallet" as IconName}]},
 {label:"Control",items:[
  {href:"/reports",label:"Laporan",icon:"chart" as IconName},{href:"/reconciliation",label:"Rekonsiliasi",icon:"arrows" as IconName},
  {href:"/periods",label:"Closing",icon:"file" as IconName},{href:"/employees",label:"Karyawan",icon:"users" as IconName}]},
...
export default function FinoraShell({children,workspaceName="Workspace",role="VIEWER",title="Finora"}:{children:React.ReactNode;workspaceName?:string;role?:string;title?:string}){
 ...
 useEffect(()=>{function onKeyDown(e:KeyboardEvent){if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="k"){e.preventDefault();searchRef.current?.focus()}if(e.key==="Escape"){setFocused(false);setNotifications(false);setMobileOpen(false)}}window.addEventListener("keydown",onKeyDown);return()=>window.removeEventListener("keydown",onKeyDown)},[])
 useEffect(()=>{let cancelled=false;Promise.all([fetch("/api/invoices"),fetch("/api/expenses")]).then(async([a,b])=>[await a.json(),await b.json()]).then(([a,b])=>{if(!cancelled)setNotificationCount((a.invoices||[]).filter((x:any)=>x.status==="OVERDUE").length+(b.expenses||[]).filter((x:any)=>x.status==="PENDING").length)}).catch(()=>{});return()=>{cancelled=true}},[pathname])
 ...
 function submitSearch(e:React.FormEvent){e.preventDefault();const x=matches[0];if(x){router.push(x.href);setSearch("");setFocused(false)}}
 return <div className="f-app">
  <aside className={mobileOpen?"f-side is-open":"f-side"}>
   <div className="f-brand"><span className="f-brand-mark" aria-hidden="true"><i/><i/><i/></span><div><b>Finora</b><small>FINANCIAL OPERATING WORKSPACE</small></div><button className="f-mobile-close" type="button" onClick={()=>setMobileOpen(false)} aria-label="Tutup menu"><Icon name="x" size={18}/></button></div>
   <div className="f-workspace" title={workspaceName}><span className="f-work-avatar">{workspaceName.slice(0,2).toUpperCase()}</span><div><small>Workspace</small><strong>{workspaceName}</strong></div></div>
   <nav className="f-nav">{navGroups.map(g=>{const visible=g.items.filter(i=>allowed(i.href));if(!visible.length)return null;return <div className="f-nav-group" key={g.label}><div className="f-nav-group-label">{g.label}</div>{visible.map(i=>{const active=pathname===i.href||(i.href!=="/dashboard"&&pathname.startsWith(i.href));return <Link key={i.href} href={i.href} title={i.label} className={active?"f-nav-item active":"f-nav-item"}><span><Icon name={i.icon}/></span><span className="f-nav-label">{i.label}</span>{i.href==="/invoices"&&notificationCount>0&&<em>{notificationCount>9?"9+":notificationCount}</em>}</Link>})}</div>})}</nav>
  ...
  <div className="f-top-left">
    ...
    <div className="f-top-context" aria-label="Lokasi halaman">
      <span className="f-top-context-kicker">Workspace</span>
      <span className="f-top-context-sep">/</span>
      <strong>{title}</strong>
    </div>

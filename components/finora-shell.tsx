...
export default function FinoraShell({children,workspaceName="Workspace",role="VIEWER",title="Finora"}:{children:React.ReactNode;workspaceName?:string;role?:string;title?:string}){
 ...
 return <div className="f-app">
  <aside className={mobileOpen?"f-side is-open":"f-side"}>
   <div className="f-brand"><span className="f-brand-mark">F</span><div><b>Finora</b><small>FINANCIAL OPERATING WORKSPACE</small></div><button className="f-mobile-close" type="button" onClick={()=>setMobileOpen(false)} aria-label="Tutup menu"><Icon name="x" size={18}/></button></div>

...
export function ProposalsManager({role}:{role:string}){
 ...
 return <div className="f-content">
  ...
  <Card><div className="f-toolbar"><input className="f-input" placeholder="Cari nomor, klien, atau project..." value={q} onChange={e=>setQ(e.target.value)}/>{message&&<span className="f-badge green">{message}</span>}</div><div style={{overflowX:'auto'}}><table className="f-table"><thead><tr><th>Proposal</th><th>Project</th><th>Klien</th><th>Valid sampai</th><th>Nilai</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{filtered.map(x=><tr key={x.id}><td><strong>{x.proposalNumber}</strong>{(x as any).quotationReference&&<div className="f-muted" style={{fontSize:11}}>Ref {(x as any).quotationReference}</div>}</td><td>{(x as any).projectName||'-'}</td><td>{x.client.name}</td><td>{new Date(x.validUntil).toLocaleDateString('id-ID')}</td><td className="f-number">{money(Number(x.totalAmount))}</td><td><Badge tone={x.status==='WON'?'green':x.status==='LOST'?'red':'blue'}>{x.status}</Badge></td><td><div style={{display:'flex',gap:6,flexWrap:'wrap'}}><a className="f-btn soft" href={`/proposals/${x.id}`}>Detail</a><a className="f-btn" href={`/documents/proposals/${x.id}`} target="_blank" rel="noreferrer">PDF / Cetak</a>{x.invoice?<a className="f-btn soft" href="/invoices">Lihat invoice</a>:<>{canWrite(role)&&x.status==='DRAFT'&&<><button className="f-btn soft" disabled={busy} onClick={()=>startEdit(x)}>Edit</button><button className="f-btn" disabled={busy} onClick={()=>remove(x.id)}>Hapus</button><button className="f-btn soft" disabled={busy} onClick={()=>action(x.id,{status:'SENT'},'Proposal dikirim.')}>Kirim</button></>}{canFinance(role)&&x.status==='SENT'&&<><button className="f-btn" disabled={busy} onClick={()=>action(x.id,{status:'NEGOTIATION'},'Proposal masuk NEGOTIATION.')}>Negosiasi</button><button className="f-btn soft" disabled={busy} onClick={()=>action(x.id,{status:'WON'},'Proposal dimenangkan.')}>Menangkan</button><button className="f-btn" disabled={busy} onClick={()=>action(x.id,{status:'LOST'},'Proposal ditetapkan LOST.')}>Tolak / LOST</button></>}</>}</div></td></tr>)}</tbody></table></div>{!filtered.length&&<div className="f-empty"><strong>Belum ada proposal</strong>Buat proposal pertama untuk memulai.</div>}</Card>
  {open&&<Modal title={editId?'Edit Proposal':'Buat Proposal'} subtitle="Gunakan struktur section/BOQ agar penawaran project lebih rapi dan mudah dikembangkan." onClose={()=>setOpen(false)}><form className="f-form f-commercial-form" onSubmit={save}>
...
function Modal({title,subtitle,onClose,children}:{title:string;subtitle?:string;onClose:()=>void;children:React.ReactNode}){
 ...
 return createPortal(<div className="f-modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div className="f-card f-modal-card" role="dialog" aria-modal="true" aria-labelledby="f-modal-title"><div className="f-modal-header"><div className="f-modal-header-copy"><h3 id="f-modal-title">{title}</h3>{subtitle&&<p>{subtitle}</p>}</div><button type="button" className="f-icon f-modal-close" onClick={onClose} aria-label="Tutup dialog">×</button></div><div className="f-modal-body">{children}</div></div></div>,document.body)}
